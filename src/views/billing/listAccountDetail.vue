    ...
    async viewFile(filePath, fileUrl) {
        try {
            ...
            const blob = new Blob([response.data], {
                type: 'application/pdf' // Account details are always PDF
            });
...
const handleDownload = async (data) => {
    ...
    try {
        ...
        const fileName = `AccountDetail_${sanitizeFileName(data.dealerName)}_${data.docsDate.replace(/\//g, '-').replace(/:/g, '-').replace(/\s/g, '_')}.pdf`;
...
const handleBulkDownload = async () => {
    ...
    try {
        ...
        for (const file of selectedFiles.value) {
            try {
                const sanitizeFileName = (name) => (name ? name.replace(/[/\\?%*:|"<>]/g, '-') : 'Unknown');
                const fileName = `AccountDetail_${sanitizeFileName(file.dealerName)}_${file.docsDate.replace(/\//g, '-').replace(/:/g, '-').replace(/\s/g, '_')}.pdf`;
