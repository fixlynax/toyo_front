<template>
    <div class="card">
        ...
        <div class="flex justify-between items-center mb-4">
            <div class="text-2xl font-bold text-black">List E-Invoice Personal - Customer: {{ custAccNo }}</div>
        </div>
                ...
                <div class="text-center py-8 text-gray-500">
                    <template v-if="!hasDateFilterApplied">
                        <div class="flex flex-col items-center gap-2">
                            ...
                            <span class="text-lg">Select a date range to view billing documents</span>
...
const BillingService = {
    async getBillingList(startDate = null, endDate = null) {
        try {
            let params = {
...
const loadFilteredData = async () => {
    ...
    try {
        listData.value = await BillingService.getBillingList(dateRange.value[0], dateRange.value[1]);
        hasDateFilterApplied.value = true;

        toast.add({
            severity: 'success',
            summary: 'Filter Applied',
            detail: `Showing billing documents from ${formatDateForDisplay(dateRange.value[0])} to ${formatDateForDisplay(dateRange.value[1])}`,
            life: 3000
        });
    } catch (err) {
